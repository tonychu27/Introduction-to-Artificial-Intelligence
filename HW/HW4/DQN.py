...
class Agent():
    ...
    def learn(self):
        ...
        # Step 3
        q_values = self.evaluate_net(observations)
        next_q_values = self.target_net(next_observations)
        value_of_now = q_values[np.arange(self.batch_size, dtype=np.int64), actions]
        value_of_future = torch.max(next_q_values, dim=1)[0]
        q_target = rewards + self.gamma * value_of_future * done

        # Step 4
        loss_function = nn.MSELoss()
        loss = loss_function(q_target, value_of_now)
