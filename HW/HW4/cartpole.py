...
class Agent():
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # raise NotImplementedError("Not implemented yet.")
        if done:
            self.qtable[tuple(state)][action]=(1.0-self.learning_rate)*self.qtable[tuple(state)][action]+self.learning_rate*reward
        else:
            self.qtable[tuple(state)][action]=(1.0-self.learning_rate)*self.qtable[tuple(state)][action]+self.learning_rate*(reward+self.gamma*np.max(self.qtable[tuple(next_state)]))
        # End your code
