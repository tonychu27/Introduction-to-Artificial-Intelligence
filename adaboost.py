...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        n_features = featureVals.shape[0]
        n_dataset = featureVals.shape[1]
        errorRate = np.zeros(n_features)

        for i in range(n_features):
            for j in range(n_dataset):
                errorRate[i] += weights[j] * \
                    abs((1 if featureVals[i][j] < 0 else 0)-labels[j])
        bestError = float('inf')
        bestClf = None
        for i in range(n_features):
            if errorRate[i] < bestError:
                bestClf = WeakClassifier(features[i])
                bestError = errorRate[i]
        # raise ImportError("To be implemented")
        # End your code (Part 2)
        return bestClf, bestError

        # Begin your code (Patr 6)
        """
        n_features = featureVals.shape[0]
        n_dataset = featureVals.shape[1]
        featureVals = np.where(featureVals < 0, 1, 0)
        bestError = float('inf')
        cnt = 0
        for i in range(n_features):
            error = 0.0
            for j in range(n_dataset):
                error = error*weights[j]*abs(featureVals[i][j]-labels[j])
                cnt = cnt+abs(featureVals[i][j]-labels[j])
            error = error**(1/cnt)
            if error < bestError:
                bestError = error
                bestClf = WeakClassifier(features[i])
